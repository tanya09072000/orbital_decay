...
function params = setup_params()
    ...
    params.start_dt      = datetime(2024, 8, 31, 23, 18, 47);
...
function results = run_simulation(params, data)
    ...
    % Физические константы для расчёта Cx
    aem = 1.6605390666e-27; % атомная единица массы, кг
    ...
    while current_time < duration_s
        ...
        alt_now = r - C.Re;
         ...
         if alt_now < 80
            fprintf('Высота %.1f км ниже 80 км — вход в атмосферу на витке %d\n', alt_now, n);
            break;
         end
        ...
        for k = 1:20
         ...
         % Cx из тех же TEMP и AMMW — без повторного вызова cira_main
            h_k  = h_grid(k);
            V_k  = sqrt(C.mu / (C.Re + h_k)) * 1000;
            m_k  = AMMW_k * aem;
            Tinf = TEMP_k(1);
            Tw   = 0.000201*h_k^3 - 0.1164*h_k^2 + 24.217*h_k - 1525;
            S    = V_k / sqrt(2*k_b*Tinf / m_k);
            Sw   = V_k / sqrt(2*k_b*Tw   / m_k);
            S1   = exp(-S^2);
            S2   = sqrt(pi) * S * (1 + 1/(2*S^2)) * erf(S);
            S3   = pi * S / Sw;
            cx_grid(k) = (2 / (sqrt(pi)*S)) * (S1 + S2 + S3);
        end
        % Интерполянты: log-пространство для rho (меняется на порядки)
        rho_interp = griddedInterpolant(h_grid, log(rho_grid), 'pchip', 'nearest');
        cx_interp  = griddedInterpolant(h_grid, cx_grid,       'pchip', 'nearest');

        % --- Интегрирование витка ---
        orbit_opts = odeset('RelTol', 1e-10, 'AbsTol', 1e-12, ...
                            'MaxStep', T_orbit/1000, 'Events', @ascendingNodeEvent);
        [~, ~, te, ye, ~] = ode89(...
            @(t, y) two_body(t, y, cx_interp, params.Sm, params.m_total, rho_interp), ...
